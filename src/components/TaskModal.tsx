...
const TimePicker12: React.FC<TimePicker12Props> = ({ id, label, value, onChange }) => {
              ...
              <div className="space-y-3 mt-1">
                ...
                {collisionResult?.hasCollision && collisionResult.conflictingItem && (
                  <div className="p-2.5 rounded-sm bg-amber-500/10 border border-amber-500/40 text-amber-900 dark:text-amber-200 text-xs font-mono flex items-start gap-2">
                    ...
                    <span>
                      Note: This time window overlaps with &apos;{collisionResult.conflictingItem.title}&apos; ({format24To12Display(collisionResult.conflictingItem.startTime)} - {format24To12Display(collisionResult.conflictingItem.endTime)}).
                    </span>
                  </div>
                )}
              </div>
